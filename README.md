...
Реализован поиск по байтовой маске (так, как я его понял) - знаком
`.` в поле для поиска можно заменить символ, значение которого неважно
(например, ячейки `f2` и `fb` подходят под маску `f.`).
